...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    d <-  ggplot(df_filter_,aes(x=Meldedatum, y=Anzahl, fill=Anteil,group=Anteil))+
      ...
      labs(title=paste('Corona-Zeitreihenanalyse (nur für Privatgebrauch)'), subtitle=sprintf("Aktive Fälle: %s   Aktuelle 7-Tage-Inzidenz: %s pro 100.000 Einwohner",
  ...
  output$plot2 <- renderPlot({ if(input$daysim > 0) plot(density(values$abw),
  ...
  output$text <- renderUI({ HTML("Disclaimer: Die Nutzung dieser Anwendung ist ausschließlich für den Privatgebrauch erlaubt.") })
