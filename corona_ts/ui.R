ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      helpText("Bitte warten, während die App initialisiert wird...\nDies kann etwa 10 Sekunden dauern.")
      ),
      conditionalPanel( # Sidepanel wird erst gezeigt, nachdem der Datensatz heruntergeladen und aufbereitet wurde
        ...
        sliderInput(inputId = "xmax", "Wie viele Tagesdaten sollen in der Grafik angezeigt werden?", min = 28, max = 350, value = 112, step = 7),
        sliderInput(inputId = "set_days_use", "Wie viele Tagesdaten sollen im Modell berücksichtigt werden?", min = 35, max = 350, value = 350, step = 7),
        ...
        sliderInput(inputId = "daysim", "Rückblick: Wie viele Tage in der Vergangenheit soll die Vorhersage beginnen?", min = 0, max = 56, value = 0, step = 7)
        )
      ),
    mainPanel(
